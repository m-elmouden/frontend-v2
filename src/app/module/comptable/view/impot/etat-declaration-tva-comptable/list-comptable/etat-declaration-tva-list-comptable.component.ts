...
@Component({
  selector: 'app-etat-declaration-tva-list-comptable',
  templateUrl: './etat-declaration-tva-list-comptable.component.html',
  styleUrls: ['./etat-declaration-tva-list-comptable.component.css']
})
export class EtatDeclarationTvaListComptableComponent implements OnInit {
   // declarations
...
public async archiverEtatDeclarationTva(etatDeclarationTva:EtatDeclarationTvaVo){
const isPermistted = await this.roleService.isPermitted('EtatDeclarationTva', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Etat declaration tva) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.etatDeclarationTvaService.archiver(etatDeclarationTva).subscribe(status=>{
const myIndex = this.etatDeclarationTvas.indexOf(etatDeclarationTva);
this.etatDeclarationTvas[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Etat declaration tva archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverEtatDeclarationTva(etatDeclarationTva:EtatDeclarationTvaVo){
const isPermistted = await this.roleService.isPermitted('EtatDeclarationTva', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Etat declaration tva) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.etatDeclarationTvaService.desarchiver(etatDeclarationTva).subscribe(status=>{
const myIndex = this.etatDeclarationTvas.indexOf(etatDeclarationTva);
this.etatDeclarationTvas[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Etat declaration tva désarchivé',
life: 3000
});
},error=>console.log(error))
 }
