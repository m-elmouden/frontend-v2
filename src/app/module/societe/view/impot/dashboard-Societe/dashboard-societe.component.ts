...
import {Product} from './api-societe/product';
import {ProductService} from './service-societe/productservice';
import {Subscription} from 'rxjs';
import {ConfigService} from './service-societe/app.config.service';
import {AppConfig} from './api-societe/appconfig';
...
@Component({
    templateUrl: './dashboard-societe.component.html',
    providers: [ProductService, ConfigService]
})
export class DashboardSocieteComponent implements OnInit {

    items: MenuItem[];

    products: Product[];

    declarations: DeclarationIrVo[];
    ...
    constructor(private productService: ProductService, public configService: ConfigService, private declaratioIrService: DeclarationIrService) {
    }

    ngOnInit() {
        this.declaratioIrService.findByEtatDeclarationIrReference('E4').subscribe(data => {
            this.declarations = data;
            console.log(this.declarations.length);
        });
        ...
        this.chartData = {
            ...
            datasets: [
                {
                    label: 'Declarations traité',
                    data: [65, 59, 80, 81, 56, 55, 40],
                    fill: false,
                    backgroundColor: '#e77707' ,
                    borderColor: '#FBC02D',
                    tension: .4
                },
                {
                    label: 'Declarations en attente',
